using System.Text;
using Blocktrust.CredentialWorkflow.Core.Domain.Credential;
using Blocktrust.CredentialWorkflow.Core.Prism;
using Blocktrust.CredentialWorkflow.Core.Services;
using FluentAssertions;

public class CredentialParserTests
{
    private readonly CredentialParser _parser;
    
    private const string ValidCredential0 = @"{
        ""@context"": [""https://www.w3.org/2018/credentials/v1""],
        ""type"": [""VerifiableCredential""],
        ""issuer"": ""did:prism:test:issuer"",
        ""issuanceDate"": ""2024-01-01T00:00:00Z"",
        ""credentialSubject"": {
            ""id"": ""did:prism:test:subject"",
            ""name"": ""Test Subject""
        }
    }";
    
        private const string ValidCredential1 = """
                                            
                                            """;

    private const string ValidCredential2 = """
                                            eyJhbGciOiJFUzI1NksifQ..RaXJtH8DXmHEEDpJMrbT8e-AxsjH4zqXeGHIxT0Ly13zzJ0GStlFkdw_SLiggQI_7iyl9xupf2icaPURcOkouw
                                            """;

    public string ValidCredential = ValidCredential1;


    public CredentialParserTests()
    {
        _parser = new CredentialParser();
    }

    [Fact]
    public void ParseCredential_WithOriginalFormat_ShouldSucceed()
    {
        // Act
        var result = _parser.ParseCredential(ValidCredential);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
        result.Value.Should().BeOfType<Credential>();
    }

    [Fact]
    public void ParseCredential_WithBase64Encoded_ShouldSucceed()
    {
        // Arrange
        var base64Credential = Convert.ToBase64String(Encoding.UTF8.GetBytes(ValidCredential));

        // Act
        var result = _parser.ParseCredential(base64Credential);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
        result.Value.Should().BeOfType<Credential>();
    }

    [Fact]
    public void ParseCredential_WithBase64UrlEncoded_ShouldSucceed()
    {
        // Arrange
        var base64UrlCredential = PrismEncoding.ByteArrayToBase64(
            Encoding.UTF8.GetBytes(ValidCredential));

        // Act
        var result = _parser.ParseCredential(base64UrlCredential);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
        result.Value.Should().BeOfType<Credential>();
    }

    [Theory]
    [InlineData(null)]
    [InlineData("")]
    [InlineData(" ")]
    public void ParseCredential_WithInvalidInput_ShouldFail(string input)
    {
        // Act
        var result = _parser.ParseCredential(input);

        // Assert
        result.IsFailed.Should().BeTrue();
    }

    [Fact]
    public void ParseCredential_WithInvalidJson_ShouldFail()
    {
        // Arrange
        var invalidJson = "{ invalid json }";

        // Act
        var result = _parser.ParseCredential(invalidJson);

        // Assert
        result.IsFailed.Should().BeTrue();
        result.Errors.Should().NotBeEmpty();
    }

    [Fact]
    public void ParseCredential_WithInvalidBase64_ShouldFail()
    {
        // Arrange
        var invalidBase64 = "!@#$%^&*()"; // Invalid base64 characters

        // Act
        var result = _parser.ParseCredential(invalidBase64);

        // Assert
        result.IsFailed.Should().BeTrue();
        result.Errors.Should().NotBeEmpty();
    }

    [Fact]
    public void ParseCredential_WithDoubleBase64Encoded_ShouldSucceed()
    {
        // Arrange
        var doubleBase64Credential = Convert.ToBase64String(
            Encoding.UTF8.GetBytes(Convert.ToBase64String(
                Encoding.UTF8.GetBytes(ValidCredential))));

        // Act
        var result = _parser.ParseCredential(doubleBase64Credential);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
        result.Value.Should().BeOfType<Credential>();
    }
}